#ifndef BTP_CONTINUOUS_OBSTACLE_BELIEF_HPP
#define BTP_CONTINUOUS_OBSTACLE_BELIEF_HPP
...
//TODO: This is mostly just a copy from obstacle_distribution
...
namespace BTP
{
    ...
    class ContinuousObstacleBelief : public Belief
    {
    public:
        GraphD graph;
        Location cur;
    ...
    public:
        ContinuousObstacleBelief(GraphD graph, Location cur) :
            graph(graph), cur(cur)
        {}

        virtual std::unique_ptr<Belief> clone() const override
        {
            std::unique_ptr<ContinuousObstacleBelief> b = std::make_unique<ContinuousObstacleBelief>(graph, cur);
            b->o = o;
            b->weights = weights;
            b->cum_sum = cum_sum;
            b->sum = sum;
